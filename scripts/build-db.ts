#!/usr/bin/env tsx
/**
 * Database builder for Philippine Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building Philippine Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Philippine statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('jurisdiction', 'PH');
