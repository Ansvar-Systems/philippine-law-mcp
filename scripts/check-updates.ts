#!/usr/bin/env tsx
/**
 * Philippine Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'https://www.officialgazette.gov.ph';
const PORTAL_NAME = 'Official Gazette (Philippines)';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/philippine-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Philippine Law MCP — Data Freshness Check');
