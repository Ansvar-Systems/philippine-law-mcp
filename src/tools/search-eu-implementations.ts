/**
 * search_eu_implementations — Search EU directives/regulations referenced by Philippine legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_philippine_implementation?: boolean;
  limit?: number;
}
...
export interface EUImplementationSearchResult {
  ...
  philippine_statute_count: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as philippine_statute_count
  ...
  if (input.has_philippine_implementation) {
    sql += ' HAVING philippine_statute_count > 0';
  }
